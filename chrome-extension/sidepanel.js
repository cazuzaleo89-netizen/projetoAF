...
function popTimerTick() {
  ...
  if (dot)  dot.classList.toggle('on', popTimerRunning);
  if (tog)  tog.textContent = popTimerRunning ? '⏸' : '▶';
  if (card) card.classList.toggle('running', popTimerRunning);
}
...
let _manHubLabel = '';
let _manHubLocal = null;  // interval para countdown local no painel
...
// ── Sparkline de tendência (taxa de acerto nos últimos dias) ─────────────────
function renderAccSparkline(weekStats) {
  const el = document.getElementById('acc-sparkline');
  const trendEl = document.getElementById('spark-trend');
  if (!el || !trendEl) return;

  const pts = (weekStats || []).filter(d => d.resolved > 0);
  if (pts.length < 2) {
    el.innerHTML = `<div style="font-size:10px;color:#1a3a5a;padding:6px 0;">Dados insuficientes</div>`;
    trendEl.className = 'spark-trend flat';
    trendEl.textContent = '—';
    return;
  }

  const W = 116, H = 34, pad = 4;
  const vals = pts.map(d => d.taxa);
  ...
  let trendCls = 'flat', arrow = '→';
  if (diff >= 4)       { trendCls = 'up';   arrow = '↗'; }
  else if (diff <= -4) { trendCls = 'down'; arrow = '↘'; }
  const lineColor = trendCls === 'up' ? '#34d399' : trendCls === 'down' ? '#fb7185' : '#7dd3fc';
  ...
  el.innerHTML = `<svg width="100%" height="${H}" viewBox="0 0 ${W} ${H}" preserveAspectRatio="none" style="display:block;overflow:visible;">
    <path d="${area}" fill="${lineColor}" opacity=".09"/>
    <path d="${line}" fill="none" stroke="${lineColor}" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/>
    <circle cx="${lastPt[0].toFixed(1)}" cy="${lastPt[1].toFixed(1)}" r="2.6" fill="${lineColor}"/>
  </svg>`;

  trendEl.className = 'spark-trend ' + trendCls;
  trendEl.textContent = `${arrow} ${diff > 0 ? '+' : ''}${diff}% em ${pts.length}d`;
}
...
function renderHoje(data) {
  ...
  const goalDim = document.getElementById('goal-progress-dim');
  if (goalDim) goalDim.textContent = resolved > 0 ? `· ${goalPct}% da meta` : '';
  ...
  // Gráfico semanal + tendência (sparkline)
  renderWeekChart(data.weekStats);
  renderAccSparkline(data.weekStats);
...
// ── Ações ────────────────────────────────────────────────────────────────────
// Nota: o painel lateral permanece aberto (não é um popup), então as ações
// abaixo não chamam window.close() — apenas focam/abrem a aba de destino.
function openPanel() {
...
async function softRefresh(force = false) {
  ...
  try {
    ...
    try { renderAccSparkline(fresh.weekStats); } catch (e) { /* */ }
